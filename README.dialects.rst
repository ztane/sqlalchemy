...
As of SQLAlchemy 0.9.4, both nose and py.test are supported for running tests,
and py.test is now preferred.
...
* setup.cfg - this file contains the traditional contents such as [egg_info],
  ...
  the SQLAlchemy nose plugin should be used.  In the case of py.test, the
  test/conftest.py file will bootstrap SQLAlchemy's plugin.

* test/conftest.py - This script bootstraps SQLAlchemy's py.test plugin
  into the py.test runner.  This
  ...
  The other portion invokes SQLAlchemy's py.test plugin::
